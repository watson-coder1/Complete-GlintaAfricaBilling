...
function system_info()
{
    ...
    function system_info_get_cpu_usage()
    {
        ...
        if (strpos($os, 'WIN') === 0) {
            ...
            if (!empty($output) && preg_match('/(\d+\.\d+)\s*id/', $output[0], $matches)) {
    ...
    function system_info_get_uptime()
    {
        ...
        if (strpos($os, 'WIN') === 0) {
            ...
            if (!empty($output)) {
                foreach ($output as $line) {
                    if (strpos($line, 'Statistics since') !== false) {
                        $uptime_string = trim(substr($line, 18));
                        $uptime_date = DateTime::createFromFormat('m/d/Y h:i:s A', $uptime_string);
                        $now = new DateTime();
                        $interval = $now->diff($uptime_date);
    ...
    function system_info_get_cpu_count()
    {
        $os = strtoupper(PHP_OS);
        ...
        if (strpos($os, 'WIN') === 0) {
            ...
            if (!empty($output) && count($output) > 1) {
                ...
                $cpu_info = explode(' ', trim($output[1]));
                $cpu_info = array_filter($cpu_info);
                $cpu_info = array_merge($cpu_info);
                return [
                    'cores' => intval($cpu_info[0]),
                    'logical_processors' => intval($cpu_info[1])
                ];
            }
        } else {
            // Linux OS
    ...
    function system_info_getSystemInfo()
    {
        ...
        $cpu_usage = system_info_get_cpu_usage();
        $uptime = system_info_get_uptime();
        $cpu_count = system_info_get_cpu_count();
        ...
        $systemInfo = [
            ...
            'CPU Usage' => $cpu_usage !== null ? $cpu_usage . '%' : 'N/A',
            'System Uptime' => $uptime,  // Added system uptime
            'CPU Count' => is_array($cpu_count) ? $cpu_count['logical_processors'] . ' (Cores: ' . $cpu_count['cores'] . ')' : $cpu_count  // Added CPU count
        ];
